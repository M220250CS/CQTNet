...
def multi_train(**kwargs):
    ...
    model = model.cuda()  # Move the model to the GPU(s)
    print(model)
    ...
    for epoch in range(opt.max_epoch):
        ...
        for (data0, label0), (data1, label1), (data2, label2) in tqdm(zip(train_dataloader0, train_dataloader1, train_dataloader2)):
            for flag in range(3):
                ...
                input = data.requires_grad_().cuda()  # Move input to GPU
                target = label.cuda()  # Move target to GPU
...
def val_slow(model, dataloader, epoch):
    ...
    for data, label in dataloader:
        input = data.cuda()  # Move input to GPU
