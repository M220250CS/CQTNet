...
def multi_train(**kwargs):
    ...
    opt.model = 'CQTNet'
    opt.notes='CQTNet'
    opt.batch_size=32  # Further reduced batch size
    opt._parse(kwargs)
    ...
    train_dataloader0 = DataLoader(train_data0, opt.batch_size, shuffle=True, num_workers=opt.num_workers, collate_fn=custom_collate_fn, pin_memory=True, prefetch_factor=2)
    train_dataloader1 = DataLoader(train_data1, opt.batch_size, shuffle=True, num_workers=opt.num_workers, collate_fn=custom_collate_fn, pin_memory=True, prefetch_factor=2)
    train_dataloader2 = DataLoader(train_data2, opt.batch_size, shuffle=True, num_workers=opt.num_workers, collate_fn=custom_collate_fn, pin_memory=True, prefetch_factor=2)
    val_dataloader = DataLoader(val_data, 1, shuffle=False, num_workers=1, collate_fn=custom_collate_fn, pin_memory=True, prefetch_factor=2)
    test_dataloader = DataLoader(test_data, 1, shuffle=False, num_workers=1, collate_fn=custom_collate_fn, pin_memory=True, prefetch_factor=2)
    val_dataloader80 = DataLoader(val_data80, 1, shuffle=False, num_workers=1, collate_fn=custom_collate_fn, pin_memory=True, prefetch_factor=2)
    ...
    for epoch in range(opt.max_epoch):
        ...
        num = 0
        optimizer.zero_grad()
        for i, ((data0, label0), (data1, label1), (data2, label2)) in enumerate(tqdm(zip(train_dataloader0, train_dataloader1, train_dataloader2))):
            for flag in range(3):
                if flag == 0:
